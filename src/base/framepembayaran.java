...
public class framepembayaran extends javax.swing.JFrame {
    ...
    public void calculateAndSavePayment(int idOrder) {
        double subtotal = getSubtotal(idOrder);
        savePayment(subtotal);
    }
    ...
    private void initComponents() {
     ...
     private double getSubtotal(int idOrder) {
        double subtotal = 0;
        String query = "SELECT SUM(harga_produk) AS subtotal FROM order_produk WHERE id_order = ?";
        try {
      ...
      private void savePayment(double subtotal) {
        double tax = subtotal * 0.005; 
        ...
        String insertQuery = "INSERT INTO payment (id_order, payment_method, subtotal, tax, discount, total_harga) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            Database db = new Database();
            db.connect();
